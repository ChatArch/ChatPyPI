...
def test_runtime_dependencies_use_current_bounds():
    ...
    assert '"chatenv>=0.2.11,<0.3.0"' in text
...
def test_chatenv_pypi_config_schema():
    ...
    assert "PYPI_SESSION_TOKEN" not in fields
...
def test_stable_profile_save_does_not_backfill_process_secrets(monkeypatch, tmp_path):
    ...
    active_path = save_active_pypi_env_value("PYPI_USERNAME", "active-user")
    profile_path = save_pypi_env_profile_value("RexWzh", "PYPI_USERNAME", "profile-user")

    active_text = active_path.read_text(encoding="utf-8")
    profile_text = profile_path.read_text(encoding="utf-8")
    assert "PYPI_USERNAME" in active_text
    assert "PYPI_USERNAME" in profile_text
