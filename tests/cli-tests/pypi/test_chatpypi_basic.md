...
测试 `chatpypi` 的基础 CLI 链路，覆盖版本输出、legacy root aliases、`pkg` 命名空间、ChatEnv token-store session 读取与手动 API token 上传入口。
...
## 用例 5：`auth session show` 支持从 ChatEnv token store 读取 session state

- 初始环境准备：
  - 已有一个由 `chatpypi auth login` 写入 `tokens/PyPI/<profile>.json` 的 PyPI web session。

预期过程和结果：
  1. 执行 `chatpypi auth session show --format json`。
  2. 预期 CLI 自动读取 ChatEnv token profile，不需要外部 session 文件或 legacy env token。
...
## 用例 7：无效 session token-store payload 必须给出明确错误

- 初始环境准备：
  - `tokens/PyPI/<profile>.json` 存在但 payload 不是 ChatPyPI 可解析的 session object。

预期过程和结果：
  1. 执行 `chatpypi auth session show`。
  2. 预期 CLI 非 0 退出。
  3. 预期错误消息明确指出 token-store payload 无效，不能抛出 traceback。
