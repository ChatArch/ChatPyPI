# test_chatpypi_init_interactive

测试 `chatpypi init` 在交互终端里缺少必要参数时，会自动进入交互补参，而不是直接报错。

## 元信息

- 命令：`chatpypi init [NAME]`
...
预期过程和结果：
1. 在交互可用条件下执行 `chatpypi init`，预期不会输出缺少包名的报错。
...
预期过程和结果：
1. 执行 `chatpypi init -I`，预期直接报错提示缺少包名。
...
2. 预期交互继续询问是否创建 mkdocs、docs domain 和 GitHub workflow 文件；仓库级域名文件不是默认模板结构。
...
预期过程和结果：
1. 执行 `chatpypi init ChatGH -t chatarch --with-chatenv-provider --chatenv-provider-name github -I`。
...
```sh
chatpypi init ChatGH -t chatarch \
